...
coxph_regression <- function(coxph_prepare) {
    ...
    ggsurvplot(survfit(Surv(duration, death) ~ PASC + COVID_positive_control + COVID_negative_control + number_of_COVID_vaccine_doses + obesity + CCI + number_of_visits_per_month_before_index_date), data = df, color = "#2E9FDF", ggtheme = theme_minimal())
    return (df)
}
